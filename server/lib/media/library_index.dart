...
class LibraryIndex {
  ...
  MediaLibraryStats stats() {
    ...
    final movies = _items.length - episodes.length;
    final shows = episodes.map((i) => i.showTitle ?? i.title).toSet().length;
    return MediaLibraryStats(movieCount: movies, showCount: shows, episodeCount: episodes.length);
  }
  ...
  List<ContinueWatchingItem> continueWatching(Map<String, double> positions, {int limit = 6}) {
    final withProgress = _items.where((i) => (positions[i.id] ?? 0) > 0).toList()
      ..sort((a, b) => (positions[b.id] ?? 0).compareTo(positions[a.id] ?? 0));
    final chosen = withProgress.isNotEmpty ? withProgress : _items;
